...
interface OrderDetailModalProps {
  ...
  onAddBlocker: (order: Order, blocker: OrderBlocker) => void;
  onResolveBlocker: (order: Order, blockerId: string) => void;
}
    ...
    >
      ...
      >
        <header className="tracker-header">
          <div>
            <p className="eyebrow">Order control · {order.id}</p>
            <div className="tracker-title-row">
              <h2 id="tracker-title">{order.customer}</h2>
        ...
        <div className="tracker-meta">
          ...
          <div>
            <span>CRF</span>
            <strong>{order.crfReference || "Pending"}</strong>
          </div>
          <div>
            <span>{order.thirdParty}</span>
            <strong>{order.thirdPartyReference || "Pending"}</strong>
          </div>
          <div>
            <span>{order.supplier}</span>
            <strong>{order.supplierReference || "Pending"}</strong>
          </div>
        </div>
        ...
        <div className="tracker-content">
                  ...
                  {openBlockers.map((blocker) => (
                    <article className="blocker-card" key={blocker.id}>
                      ...
                      <button
                        ...
                        onClick={() => onResolveBlocker(order, blocker.id)}
                        disabled={isUpdating}
                      >
                        ✓ Mark resolved
                      </button>
          ...
          <aside className="raci-panel" id="tracker-raci">
            ...
            <div className="accountability-callout">
              ...
              <p>
                FlowOps keeps {order.owner} responsible for orchestration
